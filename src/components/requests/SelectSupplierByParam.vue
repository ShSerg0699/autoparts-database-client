<template>
    <fish-form ref="selectSupplierByParam">
        ...
        <fish-fields>
            <fish-field label="Введите количество, поставленных деталей" span="8"
                        :rules="[{required: true, message: 'данное поле не должно быть пустым'}]">
                <fish-input-number v-model="parameters.quantity" hint=""></fish-input-number>
            </fish-field>
        </fish-fields>
        <fish-fields>
            <fish-field label="Период поставки данной детали" :rules="[{required: true, message: 'данное поле не должно быть пустым'}]">
                <fish-field span="12" name="date">
                    <fish-date-picker v-model="parameters.deliveryDateStart" hint="С "></fish-date-picker>
                </fish-field>
                <fish-field span="12" name="date">
                    <fish-date-picker v-model="parameters.deliveryDateEnd" hint="До"></fish-date-picker>
                </fish-field>
            </fish-field>
        </fish-fields>
        <fish-fields>
...
<script>
    ...
    export default {
        ...
        methods: {
            select: function () {
                const query = `http://localhost:8081/selectSupplierByParam` +
