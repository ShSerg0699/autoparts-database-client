<template>
    <fish-form ref="selectTopDetailInЫгзздн">
        <h2>Топ самых поставляемых деталей</h2>
        <fish-fields>
            <fish-field>
...
<script>
    ...
    export default {
        name: "select-top-detail-in-supply",
        data() {
            return {
        ...
        methods: {
            select: function () {
                const query = `http://localhost:8081/selectTopDetailInSupply`;
                axios.get(query)
