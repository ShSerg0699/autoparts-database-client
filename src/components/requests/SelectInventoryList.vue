<template>
    <fish-form ref="selectInventoryList">
        ...
        <fish-fields>
            <fish-field label="Период инвентаризации"
                        :rules="[{required: true, message: 'данное поле не должно быть пустым'}]">
                <fish-field span="12" name="date">
                    <fish-date-picker v-model="parameters.appearanceDateStart" hint="С"></fish-date-picker>
                </fish-field>
                <fish-field span="12" name="date">
                    <fish-date-picker v-model="parameters.appearanceDateEnd" hint="До"></fish-date-picker>
                </fish-field>
            </fish-field>
        </fish-fields>
        <fish-fields>
...
<script>
    ...
    export default {
        ...
        methods: {
            select: function () {
                const query = `http://localhost:8081/selectInventoryList` +
