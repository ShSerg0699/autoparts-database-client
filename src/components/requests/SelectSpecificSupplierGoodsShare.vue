<template>
    <fish-form ref="selectSpecificsSupplierGoodsShare">
        ...
        <fish-fields>
            <fish-field label="Введите ID поставщика" span="8"
                        :rules="[{required: true, message: 'данное поле не должно быть пустым'}]">
                <fish-input-number v-model="parameters.supplierID" hint=""></fish-input-number>
            </fish-field>
        </fish-fields>
        <fish-fields>
...
<script>
    ...
    export default {
        ...
        methods: {
            select: function () {
                const query = `http://localhost:8081/selectSpecificSupplierGoodsShare` +
