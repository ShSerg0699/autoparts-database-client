  ...
  <fish-form class="buyerForm" ref="getBuyer">
    ...
    <fish-fields>
      ...
      <fish-field class="idButton">
        <fish-button type="primary" @click="getBuyerById(id)"
          >Посмотреть покупателя по id</fish-button
        >
      </fish-field>
      <fish-field class="allButton">
        <fish-button type="primary" @click="getAllBuyer"
          >Посмотреть всех покупателей</fish-button
        >
      </fish-field>
    </fish-fields>
    <fish-field>
...
export default {
  ...
  methods: {
    getBuyerById: function() {
      axios
        .get("http://localhost:8081/buyer?buyerID=" + this.id)
        .then(response => (this.data = [response.data]));
    },
    getAllBuyer: function() {
      axios
        .get("http://localhost:8081/buyerAll")
        .then(response => (this.data = response.data));
    }
  }
};
