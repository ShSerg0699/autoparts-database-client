<template>
    <fish-form class="cellDropDetail">
        ...
        <fish-fields>
            <fish-field>
                <fish-button type="negative" @click="cellDropDetail">Удалить деталь</fish-button>
            </fish-field>
        </fish-fields>
        <p>{{ data }}</p>
    </fish-form>
    ...
    export default {
        ...
        methods: {
            cellDropDetail: function () {
                axios.delete("http://localhost:8081/cellDropDetail?cellID=" + this.cellID + "&detailID=" + this.detailID).then(response => (this.data = response.data))
            }
        }
    };
</script>