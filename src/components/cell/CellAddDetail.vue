<template>
    <fish-form class="cellAddDetail">
        ...
        <fish-field label="Дата появления на складе" span="8" name="date"
                    :rules="[{required: true, message: 'данное поле не должно быть пустым'}]">
            <fish-date-picker v-model="postBody.appearanceDate" hint=""></fish-date-picker>
        </fish-field>
        <fish-fields>
            <fish-field>
                <fish-button type="primary" @click="cellAddDetail">Добавить деталь</fish-button>
            </fish-field>
        </fish-fields>
        <p>{{ data }}</p>
    </fish-form>
    ...
    export default {
        ...
        methods: {
            cellAddDetail: function () {
                axios.post("http://localhost:8081/cellAddDetail?cellID=" + this.id,
                    {
