  ...
  <fish-form class="detailForm" ref="getDetail">
    ...
    <fish-fields>
      ...
      <fish-field class="idButton">
        <fish-button type="primary" @click="getDetailById(id)"
          >Посмотреть деталь по id</fish-button
        >
      </fish-field>
      <fish-field class="allButton">
        <fish-button type="primary" @click="getAllDetail"
          >Посмотреть все детали</fish-button
        >
      </fish-field>
    </fish-fields>
    <fish-table
...
export default {
  ...
  methods: {
    getDetailById: function() {
      axios
        .get("http://localhost:8081/detail?detailID=" + this.id)
        .then(response => (this.data = [response.data]));
    },
    getAllDetail: function() {
      axios
        .get("http://localhost:8081/detailAll")
        .then(response => (this.data = response.data));
    },
